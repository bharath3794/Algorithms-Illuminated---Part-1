...
func main() {
	...
	fmt.Println("-----Matrix Multiplication using Recursion-----")
	rsltMat := recMatrixMul(&mat1, &mat2)
...
*/
func recMatrixMul(v1 *array2d, v2 *array2d) *array2d{
	...
	p7 := recMatrixMul(c, f)
	p8 := recMatrixMul(d, h)
	term1 := addMatrix(p1, p2)
	term2 := addMatrix(p3, p4)
	term3 := addMatrix(p5, p6)
	term4 := addMatrix(p7, p8)
